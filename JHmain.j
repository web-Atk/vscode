library JhLib 
    ...
    function XPtraceBBB takes nothing returns nothing 
        ...
        local real dmg = LoadReal(HashBre, TmrAds, 2) 
    ...
    //浮灵攻击
    function XPtraceAAA takes unit u0, integer icUnt, real dmg, group ugp0 returns nothing 
        local unit array u1 
        local unit array u2 
        local real array u1x 
        local real array u1y 
        ...
        loop 
            ...
            set angle = Rad2Deg(angle)// 2026/8/23新增
            set u1x[i] = f1 + 80 * (Cos(angle)) 
            set u1y[i] = f2 + 80 * (Sin(angle)) 
            set u1[i] = CreateUnit(ply, icUnt, u1x[i], u1y[i], angle) 
            set r0[i] = Atan2(u2y[i] - u1y[i], u2x[i] - u1x[i]) 
            set index[0] = GetHandleId(tmr) 
            set index[1] = GetHandleId(u1[i]) 
            call UnitApplyTimedLife(u1[i], 'BHwe', 10) 
            call GroupAddUnit(ugp1, u1[i]) 
            call SaveUnitHandle(HashBre, index[1], 0, u2[i]) // 2026/8/23修改u1为u2
        ...
        call SaveReal(HashBre, index[0], 2, dmg) 
    ...
    //浮灵攻击
    function XPtraceA takes unit u0, integer icUnt, real dmg, group ugp0 returns nothing 
        local unit array u1 
        local unit array u2 
        local real array u1x 
        local real array u1y 
    ...
    function IsuseSkill takes nothing returns boolean
    ...
    function test takes unit u0 returns nothing 
        ...
        call TriggerAddCondition(tgr, function IsuseSkill)
